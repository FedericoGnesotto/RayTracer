...
bool CameraInteraction::sceneEvent(QEvent* event)
{
	auto mouseEvent = dynamic_cast<QMouseEvent*>(event);
	vec3 forwardDir = vec3::Zero();
	if (mouseEvent)
	{
		...
		{
			forwardDir = handleRotation(mouseDelta);
	...
	if (m_downKey != -1)
	{
		...
		if (positionDelta != vec3::Zero() || forwardDir != vec3::Zero())
		{
...
vec3 CameraInteraction::handleTranslation()
{
	...
	{
		return m_forwardDirection * m_speed;
	}
